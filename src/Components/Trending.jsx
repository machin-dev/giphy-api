    ...
    return(
        <div className="container-fluid ">
           <div className="shadow m-2 p-2 text-center">
            ...
            <div className="container-fluid d-flex flex-wrap justifu-content-center">
                {trend ? (
                     trend.map((t,index)=>(
                        <div className=" bg-transparent border-0 m-2" key={index} style={{width:('150px'), maxHeight:('200px')}}>
                            <img className="card-img-top rounded bg-secondary" style={{width:('100%'), height:('100%'), objectFit:('cover')}} src={t.images.original.url} alt="loading" />                  
                        </div>
                     ))
                ):(
                    <>
